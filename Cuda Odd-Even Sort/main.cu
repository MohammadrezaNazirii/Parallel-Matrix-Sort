...
__global__ void sortRows(matrix_type* matrix, int numRows, int numCols) {
    ...
    if (row < numRows) {
        for (int phase = 0; phase < numCols; ++phase) {
            for (int i = phase % 2; i < numCols - 1; i += 2) {
                int idx = row * numCols + i;
            ...
            __syncthreads();  // Ensure all threads sync after each phase
        }
    }
}
...
__global__ void sortColumns(matrix_type* matrix, int numRows, int numCols, bool* changesMade) {
    ...
    if (col < numCols) {
        ...
        for (int phase = 0; phase < numRows; ++phase) {
            for (int i = phase % 2; i < numRows - 1; i += 2) {
                int idx = i * numCols + col;
                if (matrix[idx] > matrix[(i + 1) * numCols + col]) {
                    matrix_type temp = matrix[idx];
                    matrix[idx] = matrix[(i + 1) * numCols + col];
                    matrix[(i + 1) * numCols + col] = temp;
                    localChanges = true;
                }
            }
            __syncthreads();  // Ensure all threads sync after each phase
