...
for size in "${matrix_sizes[@]}"; do
  ...
  for blocksize in "${block_sizes[@]}"; do
    echo "Running with matrix size ${size}x${size} and block size ${blocksize} with float"
     ./sort_float "matrix_double_${size}x${size}.txt" "sorted_float_${size}x${size}_${blocksize}.txt" ${blocksize} > report/report_float_${size}_${blocksize}.txt
  done
done
...
for size in "${matrix_sizes[@]}"; do
  ...
  for blocksize in "${block_sizes[@]}"; do
    echo "Running with matrix size ${size}x${size} and block size ${blocksize} with double"
     ./sort_double "matrix_double_${size}x${size}.txt" "sorted_double_${size}x${size}_${blocksize}.txt" ${blocksize} > report/report_double_${size}_${blocksize}.txt
  done
done
